  ...
  def get_quiz_next_question():
    ...
    if len(previous_questions) > 0:
      if quiz_category_id is 0: # query all categories
        next_question = Question.query.filter(~Question.id.in_(previous_questions)).first_or_404()
      else:
        next_question = Question.query.filter(Question.category == quiz_category_id, ~Question.id.in_(previous_questions)).first_or_404()
    else:
      if quiz_category_id is 0: # query all categories
        next_question = Question.query.first_or_404()
      else:
        next_question = Question.query.filter(Question.category == quiz_category_id).first_or_404()
